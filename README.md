...
| `ds.tokens.scopeStrategy` | `flat \| brand-theme` | `flat` | How directory structure under `generatedStylesRoot` maps to token scope metadata. `flat`: all files are treated as a single set with no scope (scope: {}). Use when tokens are not organised by brand or theme. `brand-theme`: path segments map to scope keys — first segment → `brand`, second → `theme` (e.g. `acme/dark/semantic.css` → scope: `{ brand: 'acme', theme: 'dark' }`). Use when tokens are organised in a `{brand}/{theme}/` directory layout. |
