...
|--------|------|---------|-------------|
| `ds.tokens.filePattern` | string \| string[] | `**/semantic.css` | Glob pattern(s) to discover token files inside `generatedStylesRoot`. Accepts a single pattern or an array of patterns. |
